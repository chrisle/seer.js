...
("ga:"!=m.slice(0,3)&&(m="ga:"+m),m="dynamic::"+m),m=encodeURIComponent(m),b=b+"&segment="+m);0<n.length&&(b=b+"&sort="+n)}catch(B){return c(B.message)}try{var C=5E3*Math.random();Utilities.sleep(C);var y=UrlFetchApp.fetch(b,{method:"get",headers:{Authorization:"GoogleLogin auth="+a,"GData-Version":"2"}})}catch(z){if(-1!=z.message.indexOf("Timeout"))y=UrlFetchApp.fetch(b,{method:"get",headers:{Authorization:"GoogleLogin auth="+a,"GData-Version":"2"}});else return c(z.message)}try{var D=y.getContentText(),
